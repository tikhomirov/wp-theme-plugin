...
if (!class_exists('WP_Bootstrap_Navwalker')) :
    class WP_Bootstrap_Navwalker extends Walker_Nav_Menu
    {
        ...
        public function start_lvl(&$output, $depth = 0, $args = null)
        {
             ...
             */
            $class_names = join(' ',
                apply_filters('nav_menu_submenu_css_class', $classes, $args,
                    $depth));
         ...
         *
         * @param  string  $output  Used to append additional content (passed by reference).
         * @param  \theme_plugin\components\WP_Nav_Menu_Item  $item  Menu item data object.
         * @param  int  $depth  Depth of menu item. Used for padding.
         * @param  WP_Nav_Menu_Args  $args  An object of wp_nav_menu() arguments.
        ...
        public function start_el(
            ...
            if (false !== strpos($args->items_wrap, 'itemscope')
             ...
             */
            $classes = $this->separate_linkmods_and_icons_from_classes($classes,
                $linkmod_classes, $icon_classes, $depth);
            ...
            $classes = apply_filters('nav_menu_css_class',
                array_filter($classes), $item, $args, $depth);
             ...
             * @param  \theme_plugin\components\WP_Nav_Menu_Item  $item  The current menu item.
             * @param  WP_Nav_Menu_Args  $args  An object of wp_nav_menu() arguments.
             * @param  int  $depth  Depth of menu item. Used for padding.
             *
             * @since WP 3.0.1
            ...
            $atts = $this->update_atts_for_linkmod_type($atts,
                $linkmod_classes);
            ...
            if ('' !== $linkmod_type) {
                ...
                $item_output .= $this->linkmod_element_open($linkmod_type,
                    $attributes);
            } else {
            ...
            if (!empty($icon_class_string)) {
                ...
                $icon_html = '<i class="'.esc_attr($icon_class_string)
                    .'" aria-hidden="true"></i> ';
            }
            ...
            $item_output .= isset($args->link_before) ? $args->link_before
                .$icon_html.$title.$args->link_after : '';
        ...
        public static function fallback($args)
        {
            ...
            // The fallback menu.
            $class = $args['menu_class'] ? ' class="menu-fallback-menu '
                .esc_attr($args['menu_class']).'"'
                : ' class="menu-fallback-menu"';
            $id = $args['menu_id'] ? ' id="'.esc_attr($args['menu_id']).'"'
                : '';
            $fallback_output .= '<ul'.$id.$class.'>';
            $fallback_output .= '<li class="nav-item"><a href="'
                .esc_url(admin_url('nav-menus.php'))
                .'" class="nav-link" title="'.esc_attr__('Add a menu',
        ...
        private function separate_linkmods_and_icons_from_classes(
            ...
            foreach ($classes as $key => $class) {
                ...
                if (preg_match('/^disabled|^sr-only/i', $class)) {
                    ...
                    $linkmod_classes[] = $class;
                    unset($classes[$key]);
                } elseif (preg_match('/^fa-(\S*)?|^fa(s|r|l|b)?(\s?)?$/i',
                    $class)
                ) {
                    // Font Awesome.
                    $icon_classes[] = $class;
                    unset($classes[$key]);
                } elseif (preg_match('/^glyphicon-(\S*)?|^glyphicon(\s?)$/i',
                    $class)
                ) {
                    // Glyphicons.
                    $icon_classes[] = $class;
